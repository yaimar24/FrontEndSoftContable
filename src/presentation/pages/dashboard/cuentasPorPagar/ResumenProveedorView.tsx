...
import type { ResumenProveedor } from '../../../../domain/models/CuentasPorPagar';
import { Users, AlertTriangle } from 'lucide-react';
import { formatCurrency } from '../../../../utils/formatters';
import { useCuentasPorPagar } from '@/application/hooks/useCuentasPorPagar';

export const ResumenProveedorView = () => {
  const { resumenProveedores, loading, fetchResumenProveedores } = useCuentasPorPagar();
  ...
  const filteredData = resumenProveedores.filter(c => {
    if (!searchTerm) return true;
    return c.proveedorNombre.toLowerCase().includes(searchTerm.toLowerCase());
  });

  const sorted = [...filteredData].sort((a, b) => b.totalDeuda - a.totalDeuda);

  const columns: Column<ResumenProveedor>[] = [
    {
      header: 'Proveedor',
      render: (item) => (
        <div className="flex items-center gap-3">
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center font-black text-sm ${item.totalVencido > 0 ? 'bg-rose-50 text-rose-600' : 'bg-blue-50 text-blue-600'}`}>
            {item.proveedorNombre.charAt(0).toUpperCase()}
          </div>
          <div className="flex flex-col">
            <span className="font-bold text-xs text-slate-800">{item.proveedorNombre}</span>
      ...
      <div className="tuto-resumen-table bg-white rounded-xl border border-slate-100 shadow-xl shadow-slate-200/40 overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-slate-400">
            ...
            <p className="mt-3 font-bold text-sm">No hay datos de proveedores</p>
          </div>
        ) : (
