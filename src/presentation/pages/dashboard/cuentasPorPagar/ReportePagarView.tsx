...
import type { ReporteCuentasPorPagarLinea } from '../../../../domain/models/CuentasPorPagar';
...
export const ReportePagarView = () => {
  const navigate = useNavigate();
  const { reporte, loading, fetchReporte } = useCuentasPorPagar();
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Factura', 'Proveedor', 'Fecha', 'Vencimiento', 'Días Vencidos', 'Total Neto', 'Pagado', 'Saldo', 'Estado', 'Rango Aging'];
    const rows = reporte.lineas.map(l => [
      l.numeroFactura, l.proveedorNombre, l.fecha.split('T')[0], l.fechaVencimiento.split('T')[0],
    ...
    a.href = url;
    a.download = `reporte_cuentasPorPagar_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  };

  const columns: Column<ReporteCuentasPorPagarLinea>[] = [
    {
      header: 'Factura',
      render: (item) => (
    ...
    {
      header: 'Proveedor',
      render: (item) => <span className="font-bold text-[10px] text-slate-700 uppercase">{item.proveedorNombre}</span>
    },
    {
      header: 'Total Neto',
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
        <div className="tuto-reporte-search w-full md:w-1/3">
          <SearchBar value={searchTerm} onChange={setSearchTerm} placeholder="Buscar por proveedor o factura..." />
        </div>
