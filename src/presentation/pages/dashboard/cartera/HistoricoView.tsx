...
export const HistoricoView = () => {
  const { historico, loading, fetchHistorico } = useCartera();
        ...
        <>
          ...
          <div className="tuto-historico-kpis grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Cartera Total al corte</p>
              <p className="text-2xl font-black text-slate-800 tracking-tight mt-1">{formatCurrency(historico.carteraTotal)}</p>
            </div>
            <div className="bg-white rounded-2xl border border-rose-100 shadow-sm p-6">
              <p className="text-[10px] font-black text-rose-400 uppercase tracking-widest">Cartera Vencida al corte</p>
              <p className="text-2xl font-black text-rose-600 tracking-tight mt-1">{formatCurrency(historico.carteraVencida)}</p>
              {historico.carteraTotal > 0 && (
                <p className="text-[10px] font-bold text-slate-400 mt-1">
                  {((historico.carteraVencida / historico.carteraTotal) * 100).toFixed(1)}% del total
                </p>
              )}
            </div>
          </div>
