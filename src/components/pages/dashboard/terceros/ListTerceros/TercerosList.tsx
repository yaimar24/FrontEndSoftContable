...
import { Search, Edit3, Mail, Phone, Building2, User, UserMinus, Link } from 'lucide-react';
...
const TercerosList: React.FC<TercerosListProps> = ({ data, onEdit }) => {
  ...
  const [confirmModal, setConfirmModal] = useState<{ show: boolean, terceroId?: string, isActivo?: boolean }>({ show: false });
  ...
  const confirmAction = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setLocalData(prev => prev.map(t => 
          t.id === confirmModal.terceroId ? { ...t, activo: !confirmModal.isActivo } : t
        ));
      }
    } catch (err: any) {
      setResultModal({ show: true, success: false, message: err.message || "Error al procesar" });
    } finally {
  ...
  const filteredData = localData.filter(t => {
    ...
    return nombreCompleto.includes(searchTerm.toLowerCase()) || t.identificacion?.includes(searchTerm);
  });
  ...
  const columns = [
    { 
      ...
      render: (t: any) => {
        ...
        const nombreAMostrar = esPersonaNatural ? `${t.nombres} ${t.apellidos}` : t.nombreComercial;
        return (
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className={`font-black tracking-tight uppercase leading-tight ${t.activo ? 'text-slate-800' : 'text-slate-400 italic'}`}>
                {nombreAMostrar || "SIN NOMBRE"}
              </span>
              
              <div className="flex items-center gap-1.5 mt-1">
                {/* Badge de Categoría original */}
                <span className={`text-[8px] font-black px-2 py-0.5 rounded-md uppercase w-fit ${t.categoriaId === 1 ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                  {t.categoriaId === 1 ? 'Cliente' : 'Proveedor'}
                </span>
                
                {/* Nuevo Badge de Estado */}
                <span className={`text-[8px] font-black px-2 py-0.5 rounded-md uppercase w-fit ${t.activo ? 'bg-blue-100 text-blue-700' : 'bg-rose-100 text-rose-700'}`}>
    ...
    { 
      ...
      render: (t: any) => (
        <div className={`flex flex-col text-[11px] font-bold text-slate-500 lowercase ${!t.activo && 'opacity-50'}`}>
          <span className="flex items-center gap-1"><Mail size={12} className="text-blue-400"/> {t.email}</span>
          <span className="flex items-center gap-1"><Phone size={12} className="text-blue-400"/> {t.telefono}</span>
        </div>
      ) 
    },
    {
  ...
  return (
    <div className="space-y-6">
      ...
      <StatusModal 
        ...
        confirmText={confirmModal.isActivo ? "Confirmar Desvinculación" : "Confirmar Vinculación"} 
        cancelText="Volver" 
      />
