...
const PerfilPage = () => {
  ...
  if (loading) return <div className="p-20 text-center font-bold text-slate-400 animate-pulse">CARGANDO...</div>;

  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-20">
      <StatusModal show={showConfirm} type="confirm" message="¿Deseas aplicar los cambios?" onConfirm={handleSave} onClose={() => setShowConfirm(false)} />
      ...
      <div className="flex justify-between items-center bg-white p-6 rounded-[2rem] border border-slate-100 sticky top-4 z-20 shadow-sm">
        ...
        <Button onClick={() => setShowConfirm(true)} icon={Save} isLoading={isSaving}>Guardar Cambios</Button>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-6">
          <SeccionDatosBasicos formData={formData} ciudades={parametros?.ciudades} onChange={handleChange} />
          <SeccionRepresentante representantes={formData.representantesLegales} tiposId={parametros?.tiposIdentificacion} onRepChange={handleRepChange} />
        </div>
        <SeccionFiscal formData={formData} parametros={parametros} onChange={handleChange} />
      </div>
    </div>
  );
};
