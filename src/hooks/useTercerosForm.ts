...
import { vincularTercero, updateTercero } from '../services/terceros/terceroService';
...
export const useTercerosForm = (token: string | null, initialData?: any) => {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        responsabilidadesFiscalesIds: initialData.responsabilidadesFiscalesIds || 
  ...
  const handleSaveClick = () => {
    const schema = {
      ...
      ...(formData.tipoPersonaId === 1 ? { nombres: [validators.required()] } : { nombreComercial: [validators.required()] }),
    };
    ...
    else { setErrors(validationErrors); window.scrollTo({ top: 0, behavior: 'smooth' }); }
  };
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      ...
      const result = initialData?.id 
        ? await updateTercero(initialData.id, payload)
  ...
  return { formData, parametros, loading, isSaving, errors, showConfirm, resultModal, setShowConfirm, setResultModal, handleChange, handleCheckboxChange, handleSaveClick, handleConfirmSave };
};